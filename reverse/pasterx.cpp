...
{
	uintptr_t ABoneArray = read<uintptr_t>( mesh + OFFSETS::BoneArray );
	if ( !ABoneArray ) ABoneArray = read<uintptr_t>( mesh + OFFSETS::BoneArray + 0x10 );
...
{
	CameraInfo camera;
	auto location_pointer = read<uintptr_t>( Uworld + 0x140 ); //110
	auto rotation_pointer = read<uintptr_t>( Uworld + 0x150 ); //120
	...
	camera.fov = read<float>( ( uintptr_t )PlayerController + 0x3AC ) * 90.f;
...
int main( int argc, const char * argv[ ] )
{
	...
	//std::rename(localpath().c_str(), random_string(15).c_str());
	...
	handler::fetch_cr3( );
	printf( " Fortnites Id Base Is: %p", ( void * )virtualaddy );
	Log3( E( " Loaded\n" ) );
	Beep( 500, 500 );
	xCreateWindow( );
	xInitD3d( );
...
void DrawESP( ) {
	...
	Uworld = read<uintptr_t>( virtualaddy + 0x1714D538 );
	...
	std::cout << LocalPlayers << std::endl;
	...
	for ( uint32_t i = 0; i < Num; i++ )
	{
		...
		auto CurrentActor = read<uintptr_t>( player + 0x320 );//PawnPrivate
	   ...
	   // if (read<float>(CurrentActor + OFFSETS::Revivefromdbnotime) != 10) continue;
		...
		int ActorTeamId = read<int>( PlayerState + OFFSETS::TeamId );
...
void render( ) {
	...
	if ( ShowMenu )
	{
		...
		ImGui::BeginChild( "##ChildIdentifier", ImVec2( 750, 500 )/*SIZE OF THE CHILD*/, false /*IF YOU WANT A BORDER OR NOT*/ );
		{
			...
			ImGui::SameLine( );
			ImGui::Text( "                  Base Address: %p", ( void * )virtualaddy );
			ImGui::Spacing( );
